...
function initializeApp() {
    ...
    document.addEventListener('calculatorModeChanged', () => {
        ...
        function updateAIGamesModalUI() {
            const modal = DOMElements.aiGamesModal;
            const screenAvailable = selection.venueScreen === 'yes' || selection.projectorNeeded;
            modal.querySelectorAll('[data-game-key]').forEach(card => {
        ...
        function setupFloatingBar() {
            ...
            openBtn.addEventListener('click', openModal); modal.addEventListener('click', (e) => { if (e.target.closest('.modal-close-btn') || e.target === modal) closeModal(); });
