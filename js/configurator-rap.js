document.addEventListener('DOMContentLoaded', () => {
 ...
 function setupEventListeners() {
  ...
  DOMElements.trackListContainer.addEventListener('click', (e) => {
 ...
 function renderTrackList() {
  ...
  DOMElements.trackListContainer.innerHTML = '';
  TRACK_LIST.forEach(track => {
   const isSelected = !!selection.selectedTracks[track.id];
   const card = document.createElement('div');
   card.className = `track-card rounded-lg p-3 ${isSelected ? 'selected' : ''}`;
   card.dataset.trackId = track.id;
   card.innerHTML = `
    <div class="flex items-center justify-between">
     <div class="flex items-center gap-3">
      <i class="fas ${isSelected ? 'fa-check-circle text-emerald-400' : 'fa-plus-circle'}"></i>
      <div>
   ...
   `;
   DOMElements.trackListContainer.appendChild(card);
  });
 }
 ...
 function formatDuration(minSeconds, maxSeconds) {
  ...
  const format = (s) => {
    ...
    parts.push(`${seconds.toString().padStart(2, '0')} сек.`);
 ...
 function generateRapPlainTextQuote() {
  ...
  function setupFloatingBar() {
    ...
    modalCopyBtn.addEventListener('click', () => {
      copyToClipboard(generateRapPlainTextQuote());
    });

    const summaryCardObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (rapCalculatorContent.classList.contains('active')) {
  ...
  function updateFloatingBarUI(total, itemsCount, currency = '₸') {
    ...
    if (window.innerWidth < 768) {
      const hostBar = document.getElementById('host-floating-summary-bar');
      if (shouldBeVisible) {
        document.body.style.paddingBottom = bar.offsetHeight + 20 + 'px';
      } else if (!hostBar || !hostBar.classList.contains('visible')) {
    ...
    if (shouldBeVisible) {
      ...
      // --- НАЧАЛО ИЗМЕНЕНИЯ ---
      // Копируем и основную смету, и уведомления в модальное окно
